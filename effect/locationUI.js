class LocationUI {
    ...
    _getLocationActions(location) {
        ...
        const actions = {
            ...
            // 中心广场
            '中心广场': [
                { key: 'promote', label: '提升好感度 (8金)', costCheck: (pl) => pl.gold >= 8 && g.chaManager.getPlayerCharacters(g.localPlayerIndex).length > 0 }
            ],
            // 冒险者公会
            '冒险者公会': [
        ...
        if (actionList.length === 0) {
            ...
            actionList.forEach(action => {
                ...
                if (!canUse) {
                    ...
                    btn.onclick = () => {
                        this._executeAction(location, action.key, playerIndex, action.label);
                        this._closeWorkspaceWindow();
                    };
                }
    ...
    _executeAction(location, actionKey, playerIndex, actionLabel) {
        const g = this.game;
        this._selectCharacterForAction(location, actionKey, playerIndex, actionLabel);
    }

    _selectCharacterForAction(location, actionKey, playerIndex, actionLabel) {
        const g = this.game;
        ...
        if (['promote', 'hotspring2', 'garden1', 'beach1', 'shopBuyAffection'].includes(actionKey)) {
        ...
        const onSelect = (charIndex) => {
            ...
            if (['promote', 'hotspring2', 'garden1', 'beach1', 'shopBuyAffection'].includes(actionKey) && (char.affection || 0) >= 5) {
    ...
    _executeWorkspaceAction(location, actionKey, playerIndex, extraParams = {}) {
        ...
        switch (actionKey) {
            case 'marketBuyWood':       actionSuccess = LocationActions.marketBuyWood(g, playerIndex).success; break;
            case 'marketBuyStone':      actionSuccess = LocationActions.marketBuyStone(g, playerIndex).success; break;
            case 'marketBuyIngredient': actionSuccess = LocationActions.marketBuyIngredient(g, playerIndex).success; break;
            case 'marketBuyMineral':    actionSuccess = LocationActions.marketBuyMineral(g, playerIndex).success; break;
            case 'lumber1':             actionSuccess = LocationActions.lumber1(g, playerIndex).success; break;
            case 'lumber2':             actionSuccess = LocationActions.lumber2(g, playerIndex).success; break;
            case 'mine1':               actionSuccess = LocationActions.mine1(g, playerIndex).success; break;
            case 'mine2':               actionSuccess = LocationActions.mine2(g, playerIndex).success; break;
            case 'forest1':             actionSuccess = LocationActions.forest1(g, playerIndex).success; break;
            case 'forest2':             actionSuccess = LocationActions.forest2(g, playerIndex).success; break;
            case 'fish1':               actionSuccess = LocationActions.fish1(g, playerIndex).success; break;
            case 'fish2':               actionSuccess = LocationActions.fish2(g, playerIndex).success; break;
            case 'restaurant1':         actionSuccess = LocationActions.restaurant1(g, playerIndex).success; break;
            case 'restaurant2':         actionSuccess = LocationActions.restaurant2(g, playerIndex).success; break;
            case 'restaurant3':         actionSuccess = LocationActions.restaurant3(g, playerIndex).success; break;
            case 'hotspring1':          actionSuccess = LocationActions.hotspring1(g, playerIndex).success; break;
            case 'hotspring2':          actionSuccess = LocationActions.hotspring2(g, playerIndex, extraParams.targetCharacterIndex || 0).success; break;
            case 'relic1':              actionSuccess = LocationActions.relicKnowledge(g, playerIndex).success; break;
            case 'port1':               actionSuccess = LocationActions.port1(g, playerIndex).success; break;
            case 'port2':               actionSuccess = LocationActions.port2(g, playerIndex).success; break;
            case 'alchemy1':            actionSuccess = LocationActions.alchemy1(g, playerIndex).success; break;
            case 'alchemy2':            actionSuccess = LocationActions.alchemy2(g, playerIndex).success; break;
            case 'garden1':             actionSuccess = LocationActions.gardenAffection(g, playerIndex, extraParams.targetCharacterIndex || 0).success; break;
            case 'beach1':              actionSuccess = LocationActions.gardenAffection(g, playerIndex, extraParams.targetCharacterIndex || 0).success; break;
            case 'workshopBuildWood':   actionSuccess = LocationActions.workshopBuildWood(g, playerIndex).success; break;
            case 'workshopBuildStone':  actionSuccess = LocationActions.workshopBuildStone(g, playerIndex).success; break;
            case 'workshopBuildMixed':  actionSuccess = LocationActions.workshopBuildMixed(g, playerIndex).success; break;
            case 'quarry1':             actionSuccess = LocationActions.quarry1(g, playerIndex).success; break;
            case 'quarry2':             actionSuccess = LocationActions.quarry2(g, playerIndex).success; break;
            case 'libraryResearchWood': actionSuccess = LocationActions.libraryResearchWood(g, playerIndex).success; break;
            case 'libraryResearchStone':actionSuccess = LocationActions.libraryResearchStone(g, playerIndex).success; break;
            case 'libraryResearchMineral':actionSuccess = LocationActions.libraryResearchMineral(g, playerIndex).success; break;
            case 'libraryLearn':        actionSuccess = LocationActions.libraryLearn(g, playerIndex).success; break;
            case 'contribute_food':     actionSuccess = LocationActions.contribute(g, playerIndex, 'food').success; break;
            case 'contribute_building': actionSuccess = LocationActions.contribute(g, playerIndex, 'building').success; break;
            case 'contribute_tool':     actionSuccess = LocationActions.contribute(g, playerIndex, 'tool').success; break;
            case 'contribute_knowledge':actionSuccess = LocationActions.contribute(g, playerIndex, 'knowledge').success; break;
            case 'promote':             actionSuccess = LocationActions.promoteAffection(g, playerIndex, extraParams.targetCharacterIndex || 0).success; break;
            case 'recruit':             actionSuccess = LocationActions.recruitCharacter(g, playerIndex).success; break;
            case 'recruitDraw':         actionSuccess = LocationActions.recruitDraw(g, playerIndex).success; break;
            case 'recruitRest':         actionSuccess = LocationActions.recruitRest(g, playerIndex).success; break;
            case 'innBuyFood':          actionSuccess = LocationActions.innBuyFood(g, playerIndex).success; break;
            case 'innBuyStamina':       actionSuccess = LocationActions.innBuyStamina(g, playerIndex).success; break;
            case 'shopBuyFood':         actionSuccess = LocationActions.shopBuyFood(g, playerIndex).success; break;
            case 'shopBuyAffection':    actionSuccess = LocationActions.shopBuyAffection(g, playerIndex, extraParams.targetCharacterIndex || 0).success; break;
            default: return;
        }

        // ✅ 修复：定义 actionMsg 并广播 Toast
        if (actionSuccess && playerName) {
            const actionLabel = extraParams.actionLabel || '工位行动';
            const actionMsg = `${playerName} 执行了「${actionLabel}」`;
            LogSystem.logAction(actionMsg, player.color);
            g.ui.showToast(actionMsg);
            g.network.broadcast({ type: 'toast', message: actionMsg });
            if (typeof soundManager !== 'undefined' && soundManager.play) {
                soundManager.play('workspace');
            }
        }
