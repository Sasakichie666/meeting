...
class NetworkEventHandler {
        ...
        switch (data.type) {
            ...
            case 'game_start':
                ...
                if (data.currentYear !== undefined) {
                    g.seasonManager.currentYear = data.currentYear;
                }

                // ✅ 关键修复：应用所有玩家资源（包含后手补偿后的金币等）
                if (data.allPlayers) {
            ...
            case 'season_change':
                ...
                g.ui.updateSeason(newSeason.icon, newSeason.name, newSeason.desc, g.seasonManager.currentYear);
                ...
                if (g.network.isHost()) {
                    ...
                    if (reqPlayer && data.playerIndex === g.currentTurnIndex) {
                        ...
                        if (result.success) {
                            ...
                            const taskDef = g.taskManager.getTaskDefinition(data.taskId);
                            const msg = `${reqPlayer.name} 完成了任务「${taskDef?.name || data.taskId}」`;
            ...
            case 'toast':
                if (data.message) {
                    g.ui.showToast(data.message);
                    // 根据消息内容自动播放对应音效
                    const msg = data.message;
                    if (typeof soundManager !== 'undefined' && soundManager.play) {
                        if (msg.includes('移动到了')) {
                            soundManager.play('move');
                        } else if (msg.includes('打出了「')) {
                            soundManager.play('cardPlay');
                        } else if (msg.includes('丢弃了「')) {
                            soundManager.play('cardDiscard');
                        } else if (msg.includes('执行了「')) {
                            soundManager.play('workspace');
                        } else if (msg.includes('完成了任务「')) {
                            soundManager.play('taskComplete');
                        } else if (msg.includes('轨道获得了')) {
                            soundManager.play('trackAdvance');
                        } else if (msg.includes('进入第') && msg.includes('年·')) {
    ...
    _handleAssignPlayer(data) {
        ...
        if (g.localPlayerIndex === null) {
            ...
            g.playerManager.ensureLocalCardBoard();
            g.seasonManager.currentYear = data.currentYear || 1;
            g.refreshUI();
        }
    }
